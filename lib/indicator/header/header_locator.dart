...
class HeaderLocator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    headerNotifier.safeOffset1 = MediaQuery.of(context).padding.top;
    return ValueListenableBuilder(
      valueListenable: headerNotifier.listenable(),
      builder: (ctx, notifier, _) {
        final headerWidget = headerNotifier.build(context);
        if (!clearExtent) {
          return _isSliver
              ? SliverToBoxAdapter(
            child: headerWidget,
        ...
        return HeaderLocatorRenderWidget(
          child: headerWidget,
          isSliver: _isSliver,
...
class HeaderLocatorRenderWidget extends SingleChildRenderObjectWidget {
  final bool isSliver;

  final double paintExtent;

  const HeaderLocatorRenderWidget({
    Key? key,
  ...
  RenderObject createRenderObject(BuildContext context) => isSliver
      ? HeaderLocatorRenderSliver(
    context: context,
    paintExtent: paintExtent,
  )
      : HeaderLocatorRenderBox(
    context: context,
    paintExtent: paintExtent,
  );
}
...
/// Use in Box
class HeaderLocatorRenderBox extends RenderProxyBox {
...
/// User in Sliver
class HeaderLocatorRenderSliver extends RenderSliverSingleBoxAdapter {
  final BuildContext context;
